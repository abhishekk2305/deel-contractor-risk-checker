...
// Database connection handled by storage module

// Complete ISO 3166-1 alpha-2 country list (~249 countries)
const fullCountriesList = [
  { iso: 'AD', name: 'Andorra' },
...
async function seedFullCountries() {
  console.log(`🌍 Seeding ${fullCountriesList.length} countries...`);
  
  let seeded = 0;
  let updated = 0;
  
  for (const countryData of fullCountriesList) {
    try {
      // Check if country already exists
      const existing = await db
        .select()
        .from(countries)
        .where(eq(countries.iso, countryData.iso))
        .limit(1);

      if (existing.length > 0) {
        // Update existing country name if different
        if (existing[0].name !== countryData.name) {
          await db
            .update(countries)
            .set({ 
              name: countryData.name,
              hasRules: false, // Default to false
        ...
        await db.insert(countries).values({
          iso: countryData.iso,
          name: countryData.name,
          hasRules: false, // Default to false, will be updated when rules are added
          lastUpdated: new Date()
        });
        seeded++;
      }
    } catch (error) {
      console.error(`❌ Error seeding ${countryData.iso} (${countryData.name}):`, error);
    }
  }

  // Get final count
  const totalCount = await db.select({ count: count() }).from(countries);
  
  console.log(`✅ Country seeding complete:`);
  console.log(`   - New countries added: ${seeded}`);
  console.log(`   - Countries updated: ${updated}`);
  console.log(`   - Total countries in database: ${totalCount[0].count}`);
}

// Count imported from drizzle-orm

if (require.main === module) {
  seedFullCountries().catch(console.error);
}

export { seedFullCountries };