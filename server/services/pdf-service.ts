...
export class PDFService {
  ...
  private createFallbackPDF(request: PDFGenerationRequest): Buffer {
    const { contractorName, riskAssessment } = request;
    const riskTier = riskAssessment.overallScore <= 30 ? 'Low' : 
                    riskAssessment.overallScore <= 70 ? 'Medium' : 'High';
    
    // Simple PDF content as a fallback
...
/F1 12 Tf
72 720 Td
(DEEL - Risk Assessment Report) Tj
0 -24 Td
(Contractor: ${contractorName}) Tj
0 -24 Td
(Risk Level: ${riskTier}) Tj
0 -24 Td
(Risk Score: ${riskAssessment.overallScore}/100) Tj
0 -24 Td
(Generated: ${new Date().toLocaleDateString()}) Tj
0 -48 Td
(This is a simplified PDF report generated) Tj
0 -12 Td
(as a fallback when full rendering is unavailable.) Tj
0 -24 Td
(Assessment ID: ${riskAssessment.id}) Tj
ET
endstream
endobj
...
%%EOF`;
  ...
  private createHTMLReport(request: PDFGenerationRequest): string {
    const { contractorName, countryName, riskAssessment } = request;
    const riskTier = riskAssessment.overallScore <= 30 ? 'Low' : 
                    riskAssessment.overallScore <= 70 ? 'Medium' : 'High';
    const riskColor = riskTier === 'Low' ? '#10B981' : 
                     riskTier === 'Medium' ? '#F59E0B' : '#EF4444';
    ...
    return `
      ...
      <head>
        <meta charset="utf-8">
        <title>Risk Assessment Report - ${contractorName}</title>
        <style>
          body { font-family: Arial, sans-serif; margin: 0; padding: 20px; color: #333; }
          .header { border-bottom: 3px solid #6366F1; padding-bottom: 20px; margin-bottom: 30px; }
          .logo { color: #6366F1; font-size: 24px; font-weight: bold; }
          .title { font-size: 20px; margin: 10px 0; }
          .risk-score { background: ${riskColor}; color: white; padding: 10px; border-radius: 5px; text-align: center; }
          .section { margin: 20px 0; }
          .section h3 { color: #6366F1; border-bottom: 1px solid #E5E7EB; padding-bottom: 5px; }
          .details { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
          .detail-item { padding: 10px; background: #F9FAFB; border-radius: 5px; }
          .footer { margin-top: 40px; font-size: 12px; color: #6B7280; text-align: center; }
        </style>
      </head>
      <body>
        <div class="header">
          <div class="logo">DEEL</div>
          <div class="title">Contractor Risk Assessment Report</div>
          <div>Generated on ${new Date().toLocaleDateString()}</div>
        </div>
        
        <div class="section">
          <div class="risk-score">
            <h2>Risk Level: ${riskTier}</h2>
            <div style="font-size: 18px;">Score: ${riskAssessment.overallScore}/100</div>
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="details">
            <div class="detail-item"><strong>Name:</strong> ${contractorName}</div>
            <div class="detail-item"><strong>Country:</strong> ${countryName}</div>
            <div class="detail-item"><strong>Risk Tier:</strong> ${riskTier}</div>
            <div class="detail-item"><strong>Assessment Date:</strong> ${new Date().toLocaleDateString()}</div>
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="details">
            <div class="detail-item">
              ...
              ${riskAssessment.factors?.sanctions ? 'Clean' : 'No issues found'}
            </div>
            <div class="detail-item">
              <strong>PEP Screening:</strong><br>
              ${riskAssessment.factors?.pep ? 'Clean' : 'No issues found'}
            </div>
            <div class="detail-item">
              <strong>Adverse Media:</strong><br>
              ${riskAssessment.factors?.adverseMedia ? 'Issues found' : 'Clean'}
            </div>
            <div class="detail-item">
              <strong>Country Risk:</strong><br>
              Based on ${countryName} risk profile
            </div>
          </div>
        </div>

        <div class="footer">
          <p>This report was generated by Deel's Global Contractor Risk Checker</p>
          <p>Report ID: ${riskAssessment.id} | Generated: ${new Date().toISOString()}</p>
        </div>
      </body>
      </html>
    `;
  ...
  async generateRiskReport(request: PDFGenerationRequest): Promise<string> {
    ...
    try {
      ...
      // Try Puppeteer first, fallback to mock if Chrome issues
      try {
        logger.info({ contractor: request.contractorName }, 'Attempting PDF generation with Puppeteer');
        ...
        try {
          ...
          logger.info({ renderTime, contractor: request.contractorName }, 'PDF render completed with Puppeteer');

        } finally {
          await browser.close();
        }
      } catch (puppeteerError) {
        logger.warn({ error: (puppeteerError as Error).message }, 'Puppeteer failed, using fallback PDF generation');
        // Create a simple fallback PDF
        pdfBuffer = this.createFallbackPDF(request);
        logger.info({ contractor: request.contractorName }, 'PDF generated using fallback method');
      }
...
%%EOF`;
      ...
      if (this.s3Client && process.env.AWS_S3_BUCKET) {
        try {
          ...
          logger.error({ error: (s3Error as Error).message || String(s3Error) }, 'S3 upload failed, using mock URL');
